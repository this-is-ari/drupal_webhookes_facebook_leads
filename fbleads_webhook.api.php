 ...
 * @return bool|NULL
 */
function hook_fbleads_webhook_access(array $payload, array $args, $event) {
 ...
 *   The GitHub Service event name, FALSE if it cannot be determined.
 *
 * @see fbleads_webhook_payload()
 * @see https://help.github.com/articles/post-receive-hooks
 * @see http://developer.github.com/v3/repos/hooks/#events
 */
function hook_fbleads_webhook_event(array $payload, array &$response, array $args, $event) {
  $commits = 0;
  foreach ($payload['commits'] as $commit) {
    $commits++;
    watchdog('fbleads_webhook', t('Commit @id pushed to repo @repo'), array(
      '@id' => $commit['id'],
      '@repo' => $commit['url'],
    ));
  }
